...
#        aligned_reads_line = lines[4].split('\t')[0]
        # Extraire le nombre de reads align align√
...
def extract_mapping_info(file):
    with open(file, 'r') as f:
        lines = f.readlines()
        if len(lines) < 5:
            print("Le fichier ne contient pas toutes les informations necessaire")
...
def write_to_csv(file, data):
    with open(file, 'w', newline='') as f:
        ...
        data_sorted = sorted(data, key=lambda x: x[0])  # trier les donn
...
#rediriger les path afin de l'adapter avec la logique snakemake:
   ...
   inputPath1=args.flagstats_directory[0]
   inputPath2=args.coverage_directory[0]
...
#extraire le nom d'echantillon du premier fichier
   ...
   flagstat_files = glob.glob(flagstats_directory1 + "/*.stats.txt")
   if not flagstat_files:
      print("No flagstat files found in " + flagstats_directory1)
      sys.exit(1)

   samplename = os.path.basename(flagstat_files[0]).split(".")[1]
   print("Processing files for " + samplename)

#process all files in input directory 
   data = []
   for filename1 in sorted(os.listdir(flagstats_directory1)):
      if filename1.endswith("stats.txt"):
         ...
         print("Processed " + flagstats_file+ " and " + coverage_file)
#   data = []
